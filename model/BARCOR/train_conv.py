...
from transformers import AdamW, get_linear_schedule_with_warmup, AutoTokenizer, BartConfig, AutoModelForSeq2SeqLM
from transformers.trainer_pt_utils import get_parameter_names

from dataloader_bart import BARTDataCollatorForConv
...
def evaluation(dataloader):
    ...
    for batch in tqdm(dataloader, disable=not accelerator.is_local_main_process):
        ...
        gen_args = {
            'min_length': 0,
            'max_length': args.resp_max_length,
            'num_beams': 1,
            'no_repeat_ngram_size': 3,
            # 'encoder_no_repeat_ngram_size': 3
        }
        gen_seqs = accelerator.unwrap_model(conv_model).generate(**batch, **gen_args)
...
if __name__ == '__main__':
    ...
    conv_model = AutoModelForSeq2SeqLM.from_pretrained(args.conv_model).to(device)
    ...
    train_dataset = DatasetForConv(
        data_list=train_data_list, tokenizer=tokenizer, entity2id=kg['entity2id'], debug=args.debug, shot=args.shot,
    ...
    if args.other_dataset is not None:
        ...
        other_dataset = DatasetForConv(
            data_list=other_data_list, tokenizer=tokenizer, entity2id=kg['entity2id'], debug=args.debug,
            context_max_length=args.context_max_length, resp_max_length=args.resp_max_length,
        )
    ...
    data_collator = BARTDataCollatorForConv(
        tokenizer=tokenizer, device=device, debug=args.debug, use_amp=accelerator.use_fp16,
        context_max_length=args.context_max_length, resp_max_length=args.resp_max_length,
    )
