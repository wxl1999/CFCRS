...
class ConvEvaluator:
    ...
    def evaluate(self, preds, labels, log=False):
        ...
        decoded_preds = [decoded_pred.replace('<pad>', '') for decoded_pred in
                         decoded_preds]
        decoded_preds = [pred.strip() for pred in decoded_preds]
        decoded_labels = self.tokenizer.batch_decode(labels, skip_special_tokens=False)
        decoded_labels = [decoded_label.replace('<pad>', '') for decoded_label in
                          decoded_labels]
        ...
        decoded_preds = [decoded_pred.replace('<|endoftext|>', '') for decoded_pred in
                         decoded_preds]
        decoded_labels = [decoded_label.replace('<|endoftext|>', '') for decoded_label in
                          decoded_labels]
