...
from dataloader_unicrs import UniCRSDataCollatorForConv
...
def evaluation(dataloader):
    ...
    for batch in tqdm(dataloader, disable=not accelerator.is_local_main_process):
        batch = data_collator_for_teaching(batch)
        ...
        loss = model(**batch['context'], conv=True).conv_loss
        loss_list.append(float(loss))

    for batch in tqdm(dataloader, disable=not accelerator.is_local_main_process):
        batch = data_collator_for_gen(batch)
        token_embeds = text_encoder(**batch['prompt']).last_hidden_state
        prompt_embeds = prompt_encoder(
            entity_ids=batch['entity'],
            token_embeds=token_embeds,
        ...
        if args.debug:
            gen_args['min_length'] = args.context_max_length + args.resp_max_length

        gen_seqs = accelerator.unwrap_model(model).generate(**batch['context'], **gen_args)
        gen_resp_ids = []
        for gen_seq, length in zip(gen_seqs, batch['context_len']):
            gen_seq = [token_id for token_id in gen_seq if token_id != tokenizer.pad_token_id]
            gen_resp_ids.append(gen_seq[length:])
        evaluator.evaluate(gen_resp_ids, batch['conv_labels'], log=accelerator.is_local_main_process)
...
def learning():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if report['loss'] < best_metric_val:
            ...
            prompt_encoder.save(best_dir)
            logger.info(f'save model with best loss')
...
if __name__ == '__main__':
    ...
    # kg
    kg = KGForUniCRS(kg=args.kg_dataset, debug=args.debug).get_kg_info()
    ...
    train_dataset = DatasetForConv(
        data_list=train_data_list, tokenizer=tokenizer, prompt_tokenizer=text_tokenizer,
    ...
    if args.other_dataset is not None:
        ...
        other_dataset = DatasetForConv(
            data_list=other_data_list, tokenizer=tokenizer, prompt_tokenizer=text_tokenizer,
            debug=args.debug,
            context_max_length=args.context_max_length, resp_max_length=args.resp_max_length,
            entity2id=kg['entity2id'], entity_max_length=args.entity_max_length
        )
    ...
    valid_dataset = DatasetForConv(
        data_list=valid_data_list, tokenizer=tokenizer, prompt_tokenizer=text_tokenizer,
        debug=args.debug,
        context_max_length=args.context_max_length, resp_max_length=args.resp_max_length,
        entity2id=kg['entity2id'], entity_max_length=args.entity_max_length
    )
    ...
    test_dataset = DatasetForConv(
        data_list=test_data_list, tokenizer=tokenizer, prompt_tokenizer=text_tokenizer,
        debug=args.debug,
        context_max_length=args.context_max_length, resp_max_length=args.resp_max_length,
        entity2id=kg['entity2id'], entity_max_length=args.entity_max_length
    )
    ...
    data_collator_for_teaching = UniCRSDataCollatorForConv(
        tokenizer=tokenizer, prompt_tokenizer=text_tokenizer,
        device=device, debug=args.debug, use_amp=accelerator.use_fp16, gen=False,
        context_max_length=args.context_max_length, resp_max_length=args.resp_max_length,
        entity_pad_id=kg['pad_entity_id'], entity_max_length=args.entity_max_length
    )
    data_collator_for_gen = UniCRSDataCollatorForConv(
        tokenizer=tokenizer, prompt_tokenizer=text_tokenizer,
        device=device, debug=args.debug, use_amp=accelerator.use_fp16, gen=True,
        context_max_length=args.context_max_length, resp_max_length=args.resp_max_length,
        entity_pad_id=kg['pad_entity_id'], entity_max_length=args.entity_max_length
    )
    ...
    evaluator = ConvEvaluator(tokenizer=tokenizer, log_file_path=log_file_path)
