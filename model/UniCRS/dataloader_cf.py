...
class UserDataCollator:
    def __init__(self, kg, user_entity_max_length=100, device=torch.device('cpu'), debug=False):
    ...
    def __call__(self, data_batch):
        ...
        user_entity_ids = padded_tensor(
            ...
            max_length=self.user_entity_max_length, debug=self.debug
        )
...
class FlowDataCollator:
    def __init__(self, kg, use_meta_path=True, max_length=100, device=torch.device('cpu'), debug=False):
        self.device = device
        self.debug = debug
