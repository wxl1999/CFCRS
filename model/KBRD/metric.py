...
class ConvMetric:
    ...
    def evaluate(self, preds, labels, log=False):
        ...
        decoded_preds = self.tokenizer.batch_decode(preds, skip_special_tokens=False)
        decoded_preds = [
            pred.replace(f'{self.tokenizer.pad_token}', '').replace(f'{self.tokenizer.eos_token}', '').strip()
            for pred in decoded_preds
        ]
        decoded_labels = self.tokenizer.batch_decode(labels, skip_special_tokens=False)
        decoded_labels = [
            label.replace(f'{self.tokenizer.pad_token}', '').replace(f'{self.tokenizer.eos_token}', '').strip()
